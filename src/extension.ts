...
import antdComponents, { AntdComponent } from './config/components';

type Component = AntdComponent & {path: string};

const components: Component[] = [];
Object.keys(antdComponents).forEach(item => {
    components.push({
        ...antdComponents[item],
        path: item,
    });
...
export function activate(context: vscode.ExtensionContext) {
    ...
    let registration = vscode.workspace.registerTextDocumentContentProvider('lib-wizard', docs);
    ...
    let disposable = vscode.commands.registerCommand('lib-wizard.search', () => {
        // if (context.workspaceState.get('lib-wizard.loading', false)) {
