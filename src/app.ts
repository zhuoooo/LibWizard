...
import {uiTags} from './config/uiTags';
import ATTRS from './config/uiAttributes';
...
export class AntdvCompletionItemProvider implements CompletionItemProvider {
  ...
  getTagSuggestion() {
    ...
    let id = 100;
    for (let tag in uiTags) {
      suggestions.push(this.buildTagSuggestion(tag, uiTags[tag], id));
      id++;
    }
    return suggestions;
  }
  ...
  getAttrSuggestion(tag: string = ''): any[] {
    let suggestions: any[] = [];
  ...
  buildTagSuggestion(tag: any, tagVal: any, id: any) {
    ...
    function build(tag: any, { subtags, defaults }: any, snippets: any[]) {
      ...
      index++;
      subtags && subtags.forEach((item: string | number) => build(item, uiTags[item], snippets));
      snippets.push(`</${tag}>`);
    };
  ...
  buildAttrSuggestion({ attr, tag, bind, method }: any, { description, type, optionType, defaultValue }: any) {
  ...
  getTagAttrs(tag: string) {
    return (uiTags[tag] && uiTags[tag].attributes) || [];
  }
